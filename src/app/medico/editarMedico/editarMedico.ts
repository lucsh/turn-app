...
export class EditarMedicoComponent implements OnInit, OnChanges {
  ...
  */
  ...
  public actualizarDatos(nombre, apellido, email, duracionTurno) {
    ...
    const idUsuario = this.medicoSeleccionado._idUsuario;
    const emailMedico = email.toLowerCase();

    const yo = this;

    const obrasAsignadas = this.asignarObras();

    this.configuracionMedicoService.actualizarMedico(id, nombre, apellido, emailMedico, duracionTurno, obrasAsignadas, idUsuario)
      .then(medicoNuevo => {
